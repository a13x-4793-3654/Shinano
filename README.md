...
共有秘密鍵は main process だけが復号し、後述の専用ファイルに **Electron safeStorage と OS の鍵で暗号化して保存**します。保存済みの平文秘密鍵を UI へ返す機能、秘密鍵のエクスポート、QR スキャン、クラウド同期、他 Authenticator からの抽出はありません。暗号化が利用できない場合は日本語のエラーを表示し、平文や固定パスワードへフォールバックしません。破損・未対応の形式も自動初期化せず、対象を確認して削除・再登録するまで保持します。
...
プロファイル削除はネイティブの確認ダイアログを経て、そのプロファイルの全タブを閉じ、ローカルの Cookie・サイトストレージ・キャッシュ・HTTP 認証キャッシュ・TOTP 登録を消去します。先に削除予定の ID を保存し、Electron が保持するファイルハンドルとの競合を避けるため、残存するプロファイルディレクトリは**次回起動時、セッションを開く前**に除去します。TOTP ファイル・そのプロファイルの未完了の一時保存ファイル・browser partition の処理が完了するまで削除予定を保持します。暗号文の削除には OS 鍵の復号アクセスは不要です。途中で失敗した場合も明示して再試行します。未完了の TOTP 一時保存や復号できない既存登録を通常の置換で上書きせず、必要な場合は対象の登録を確認して削除してから再登録します。
